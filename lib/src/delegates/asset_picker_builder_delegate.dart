...
abstract class AssetPickerBuilderDelegate<Asset, Path> {
  ...
  late final permissionOverlayDisplay = ValueNotifier<bool>(
    limitedPermissionOverlayPredicate?.call(permissionNotifier.value) ?? (permissionNotifier.value == PermissionState.limited),
  );
      ...
      child: IconButton(
        ...
        icon: Icon(
          ...
          semanticLabel: switch (Theme.of(context).platform) {
            TargetPlatform.android => MaterialLocalizations.of(context).closeButtonTooltip,
            _ => null,
          },
        ),
      ),
...
class DefaultAssetPickerBuilderDelegate extends AssetPickerBuilderDelegate<AssetEntity, AssetPathEntity> {
  ...
  /// 拖拽选择协调器
  late final AssetGridDragSelectionCoordinator dragSelectCoordinator = AssetGridDragSelectionCoordinator(delegate: this);
          ...
          return SliverGrid(
            delegate: SliverChildBuilderDelegate(
              (context, int index) {
                ...
                if (dragToSelect ?? !MediaQuery.accessibleNavigationOf(context)) {
                ...
                child: Selector<DefaultAssetPickerProvider, List<AssetEntity>>(
                  ...
                  builder: (BuildContext context, List<AssetEntity> assets, _) {
                    ...
                    return ScrollbarUltima.semicircle(
                      ...
                      thumbMainAxisSize: 42,
                      thumbCrossAxisSize: 25,
  ...
  Widget itemBannedIndicator(BuildContext context, AssetEntity asset) {
    return Consumer<DefaultAssetPickerProvider>(
      builder: (_, DefaultAssetPickerProvider p, __) {
        // 不选视频时长超过5分钟的 音频超过10分钟的
        final bool isDisabled = (!p.selectedAssets.contains(asset) && p.selectedMaximumAssets) || (isWeChatMoment && asset.type == AssetType.video && p.selectedAssets.isNotEmpty) || (asset.type == AssetType.video && asset.videoDuration.inMilliseconds > 60000 * 5) || (asset.type == AssetType.audio && asset.videoDuration.inMilliseconds > 60000 * 10);
        if (isDisabled) {
          return Container(
            color: theme.colorScheme.background.withOpacity(.85),
